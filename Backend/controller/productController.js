...
async function showById(req, res) {
    ...
    try {
        ...
        // yong rows sya yong tatanggap ng result ir meron man
        if (rows.length > 0) { // check if the rows.length is greater than 0 if true retun the value or the rows
            return res.json({
                success: true,
                data: rows
            })
        }
        // if the condation is false then return this error message 
...
async function updateProduct(req, res) {
    ...
    const { product_name, price, des } = req.body; // destructuring the inputs from body 
    const { id } = req.params;


    try {
        await productModel.update(product_name, price, des, id)
